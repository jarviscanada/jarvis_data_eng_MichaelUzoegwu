...
public class BST<K extends Comparable<K>, V> {
  ...
  /**
   * Traverse through the binary search Tree and return the nodes in a particular order
   * specified by {@code mode}.
   * 0 -> pre-order | 1 -> post-order | 2 -> in-order
  ...
  private void traverseRecursive(Node<K,V> node, List<V> order, int mode){
    ...
    if (mode == 0) order.add(node.value);
    traverseRecursive(node.left, order, mode);
    if (mode == 2) order.add(node.value);
    traverseRecursive(node.right, order, mode);
    if (mode == 1) order.add(node.value);
  }
